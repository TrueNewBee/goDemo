...
// ini配置文件解析器 go  ini  可以封装发到GitHub
...
// 全是用反射实现的  go  ini  可以封装发到GitHub
func loadIni(fileName string, data interface{}) (err error) {
	// 0. 参数的校验
	// 0.1 传进来的data参数必须是指针类型(因为所需要在函数中对其赋值)
	t := reflect.TypeOf(data) // 识别传入了类型
	...
	// 0.2 传进来的data参数必须是结构体类型指针(因为配置文件中各种键值队需要赋值给结构体的字段)
	...
	for idx, line := range lineSlice {
		...
		if strings.HasPrefix(line, "[") {
			...
			//把这一个行首尾的[]去掉,取到中间的内容把首尾的空格去掉拿到内容
			sectionName := strings.TrimSpace(line[1 : len(line)-1])
			if len(sectionName) == 0 {
				err = fmt.Errorf("line:%d syntax error", idx+1)
				return
			}
			// 根据字符串 sectionName去data里面根据反射去找对应的结构体
			...
			// 2. 根据structName 去 data里面把对应的嵌套结构体给取出
			v := reflect.ValueOf(data)
			sValue := v.Elem().FieldByName(structName) // 拿到嵌套结构体值信息
			sType := sValue.Type()                     // 拿到嵌套结构体类型信息
			...
			for i := 0; i < sValue.NumField(); i++ {
				field := sType.Field(i) // tagx信息是存储在类型信息中
				fieldType = field
